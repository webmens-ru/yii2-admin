...
use wm\admin\jobs\historyLead\HistoryLeadSynchronizationFullListJob;
use wm\admin\jobs\historyLead\HistoryLeadSynchronizationFullGetJob;
use wm\admin\jobs\historyLead\HistoryLeadSynchronizationDeltaJob;
...
class HistoryLead extends BaseEntity implements SynchronizationInterface
{
    ...
    public static function getB24Fields()
    {
        return [
            ...
            "STATUS_SEMANTIC_ID" => "Семантика статуса P - промежуточная стадия, S - успешная стадия, F - провальная стадия (стадии)",
    ...
    public static function startSynchronization($modelAgentTimeSettings)
    {
        ...
        if (!$agent) {
            $agent = new Agents();
            $agent->name = 'Синхронизация дельты история(лид)';
...
//        $agent->period = $period;
    ...
    public function loadData($data)
    {
        ...
        $this->save();
        if ($this->errors) {
            Yii::error($this->errors, 'HistoryLead->loadData()');
        }
    }
